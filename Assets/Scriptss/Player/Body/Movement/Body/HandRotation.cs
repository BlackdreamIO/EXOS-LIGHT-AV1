...
public class HandRotation : MonoBehaviour
{
    ...
    private void Update()
    {
        if(playerAccessPoint.Doing == PlayerAccessPoint.IsDoing.Interaction)
        {
            Debug.Log("Player Is Interacting With An Object Cant Do Hand Movement");
            return;
        }
        HandleMouseInput();
        ManageHandMovement();
    ...
    private void ManageHandMovement()
    {
        ...
        {
            isRotating = true;
            transform.localRotation = Quaternion.Lerp(A, B, m_smooth * Time.deltaTime);
        }
        else
        {
            isRotating = false;
